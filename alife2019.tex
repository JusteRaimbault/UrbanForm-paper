...
% - The conference venue seems to be a perfect example to introduce issues linked to urban systems
% - First it features the "Urban Science Building", what is a rather new concept (cf Mike Batty book), although ideas and contributions in that field already date back in the several related disciplines.
% - Secondly, when looking at the website of the project, several keywords could be typical of ALife abstracts : innovation, sustainiblity, biological metaphor, regenration.
% - It is therefore natural to ask the question to what extent are cities alive. Naturally they are not living organism, but are highly complex, and imply complex interplay between the form and the function,  i.e. morphogenesis. 
...
% TODO finish paper morphogenesis
...
% - When considering urban systems, several scales and objects can be the focus of models.
% - for example on the left, urban sprawl can be related to the extension of road networks, with simple parameters already producing a large varieties of urban dynamical regimes
% - also, in relation with morphogenesis, it can be shown that reaction-diffusion processes for population distribution reproduce existing settelements at an intermediate scale with a relatively high accuracy.
...
% - furthermore, other scales can be explored, as on the left for a co-evolution model between cities and transportation networks at the scale of the system of cities.
% - similarly, other dimensions, such as transportation networks themselves, can be the subject of urban morphogenesis models.
...
% - We propose in this contribution to follow these ideas and to investigate the emergence of the urban form from the bottom-up.
% - Large geographical scales remain not that much investigated by these fields (alife and quant geo - they are by urban design, architecture, procedural modeling)
% - Similarly, sets of indicators to capture features of urban form are lacking in the literature.
...
% - Following the very simplified narrative that cities are the historical superposition of very different building processes, in space and time, we propopse to investigate different types of processes
% - the first one is just a distribution of building blocks, which has been typical to modern urbanism (folowing Le corbusier) - in the generator, blocks of random height and width are distributed at random
% - it is then well known that kernel mixture are perfformant to reproduce population distributions. At large scales, we make the assumption that similare aggrgegatipon patterns shoudl happen, but as we deal with binary structures, we use thresholded exponential mixtures
% - finally, we can also consider the flows within the city - in the case of a very dense urban center, we can assume that it was however constrained to allow a minimal quantity of flows to go through. Therefore, we use network percolation to produce "labyrinth-like" urban conifgurations (percolate a grid network until a fixed number of outputs are joined in a giant cluster).
...
% Then comes the issue of quantifying urban form.
% again, the literature is very diverse and depends on disciplines and objectives.
% we porpose a multi indicator set based on complementary assumptions and processes.
% - first summary statistics of built environment distribution
% - then spatial statistics capturing a level of agregation
% - a network based detour index capturing a level of sinuosity
% - mathemtaical morphology indicators are good proxies to capture the size of open spaces and buildings
...
% - we show here examples of generated urban forms
% - these are parameter sets that can be more or less close to real urban configuration
% - the random generator is a null model to control that results on morphological indicators are indeed significant
...
% - one of our important contribution is to confront models to a large set of real configurations
% - european urban areas were randomly sampled (weighted by population)
% - square areas were randomly sampled within these (72,000 reuqests on OpenStreetMap - requires to locally load OSM data into a pgsql server)
...
% - real configuration can be classified with unsupervised clustering
% - can see here four classes ranging from large / small blocks, intermediate shapes, more compact configurations.
% - we find that effective dimension is not very high (higher than at meso scale) - could not know before which indicators would be discriminant)
...
% - we then use high performance computing to run generators a large number of times
% - therefore, the OpenMOLE software was used
% - workflow engine for computational experiments, embeds any model, provide transparent access to computation clusters and grids, and provide model exploration methods (sensitivity analysis, doe, GA, Diversity search)
% - you can download it for free, see the doc etc here.
...
% - these are results - 10,000 parameter points were randomly sampled for each geenrator - 100 stochatsic repetitions for each (I recall that each generator has three parameters)
% - these are visual outputs of close real and simulated configurations. for some it seems "easy" to be visually close, for others more difficult. ad hoc modifications could be done to be closer, but as we will see the generators are already satisfying in terms of morphological proximity
...
% - so this is the comparison of the real point cloud with simulated ones
% - the random control is in purple and confirms that indicators capture relevant forms
% - the blue corresponds to percolation, red blocks and green exp mixture
% - a significant proportion of real points (dark blue) is covered - and all generators are necessary - a range is still out of reach, further work would consist in checking if intrinsic to generators or to the exploration of parameter space used
...
% when agregating on repetitions, we can get the distances for each centroid and each generator, and confirm the small minimal distances and their complementarity
...
% - several developments remain to be done, such as more elaborated methdos to calibraate and explore model behavior ; dynamical calibration is also a difficult issue, limited by the availibility of data ; also we had here a fair comparison of generators, but varying number of parameters is an other issue to tackle if it is the case.
% - regarding applications, beginning to include processes regarding mobility, pollution, co2 emissions, and linking them to urban form, is an important direction towards sustainability. this imply multidimensional and multiscalar models, and finally the integqartion into operational approches - whats typical in urban science is the close link between research and practice.
%
...
% - to sum up, we have introduced a newt set of urban morphology measures at large scales, and calibrated simple generative models on real configurations.
% - this can be understood as a first step towards a more systematic understanding of urban morphogenesis.
% - everything is openly available on the github repository, and you can check out openmole at this adress.
